...
# Define search field enum
class SearchField(StrEnum):
    """Available fields for user search."""
    name = "name"
    surname = "surname"
    email = "email"
...
class SearchRequest(BaseModel):
    """Single search request with field and value."""
...
def retrieve_context(user_question: str) -> list[dict[str, Any]]:
    ...
    # 5. If search parameters found, search users
    ...
    # 6. No specific search parameters found
...
def augment_prompt(user_question: str, context: list[dict[str, Any]]) -> str:
    ...
    # 1. Prepare context from users JSONs as string
    context_lines = []
    for user in context:
        context_lines.append("User:")
        for key, value in user.items():
            context_lines.append(f"  {key}: {value}")
        context_lines.append("")  # Add blank line between users
    context_str = "\n".join(context_lines)
    ...
    print(f"\n📝 Augmented prompt created with {len(context)} user(s)")
...
def main():
    ...
    while True:
        ...
        if context:
            ...
            # 5. Otherwise print no information found
